...
export function InteractiveMap({ variant = 'default', className = '' }: InteractiveMapProps) {
  ...
  return (
    <section id="map" className="py-24 bg-gradient-to-b from-white to-teal-50">
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
          <div className="flex items-center justify-center gap-2 mb-4">
            <Navigation className="w-6 h-6 text-teal-600 animate-bounce" />
            <span className="text-teal-600 font-bold uppercase tracking-[4px]">Regional Navigator</span>
          </div>
          <h2 className="text-5xl md:text-6xl mb-6 text-teal-950 font-bold tracking-tight">Geographical <span className="text-teal-600">Treasures</span></h2>
          <p className="text-xl text-teal-800 max-w-2xl mx-auto leading-relaxed font-medium">Explore the official provinces and top destinations of Eastern Morocco with our high-precision interactive map.</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          <Card className="max-w-4xl mx-auto border-teal-200/50 shadow-[0_32px_64px_-16px_rgba(20,184,166,0.15)] overflow-hidden rounded-[40px] bg-white ring-1 ring-teal-50">
            ...
            <CardContent className="p-0 bg-cyan-50/20">
              <div className="relative w-full aspect-square md:aspect-video flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(#14b8a6 1px, transparent 1px)', backgroundSize: '20px 20px' }}></div>
        ...
        <AnimatePresence>
          {selectedDestination && (
            <Dialog open={!!selectedDestination} onOpenChange={() => setSelectedDestination(null)}>
              <DialogContent className="max-w-2xl rounded-[40px] overflow-hidden p-0 border-none shadow-2xl">
                <div className="relative h-80">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex items-end p-10">
                    <div className="flex justify-between items-end w-full">
                      ...
                      <Badge className="bg-teal-500 text-white text-xl py-2 px-4 rounded-2xl flex items-center gap-2">
                        <Star className="w-5 h-5 fill-white" /> {selectedDestination.rating}
                      </Badge>
                    </div>
                  </div>
                </div>
                <div className="p-10 bg-white">
                  <p className="text-teal-800 text-lg leading-relaxed mb-8">{selectedDestination.description}</p>
                  <div className="space-y-4 mb-8">
                    <h4 className="font-bold text-teal-950 uppercase tracking-widest text-sm">Highlights</h4>
                    <div className="flex flex-wrap gap-2">
                      {selectedDestination.highlights.map((h, i) => (
                        <span key={i} className="bg-teal-50 text-teal-700 px-4 py-2 rounded-full text-sm font-bold border border-teal-100">{h}</span>
                      ))}
                    </div>
                  </div>
                  <div className="flex gap-4">
                    <Button className="flex-1 h-14 rounded-2xl bg-teal-600 hover:bg-teal-700 text-white font-bold text-lg shadow-xl shadow-teal-500/30">Plan This Visit</Button>
                    <Button variant="outline" onClick={() => setSelectedDestination(null)} className="px-8 h-14 rounded-2xl border-teal-200 text-teal-900 font-bold">Close</Button>
                  </div>
                </div>
              </DialogContent>
            </Dialog>
        ...
        <div className="grid md:grid-cols-3 gap-8 mt-20 max-w-5xl mx-auto">
          {[
            ...
            <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
              <div className="bg-white p-8 rounded-3xl border border-teal-100 hover:border-teal-500 transition-all text-center shadow-sm hover:shadow-xl group">
                <div className="text-2xl font-bold text-teal-600 mb-2 group-hover:scale-110 transition-transform">{stat.value}</div>
                <p className="text-teal-800 font-medium text-sm leading-snug">{stat.label}</p>
              </div>
            </motion.div>
          ))}
        </div>
