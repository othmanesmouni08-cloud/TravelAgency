      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {(activities || []).filter(a => !!a && !!a.id).map((activity) => {
            ...
            // Image mapping helper for dynamic content from DB
            const getImagePath = (item: any) => {
              // Safety check: if name is missing, return fallback immediately
              if (!item || !item.name) return '/images/placeholder.jpg';

              // Normalize name to remove accents (e.g., saïdia -> saidia)
              ...
              // Fallback to original path if no match found
